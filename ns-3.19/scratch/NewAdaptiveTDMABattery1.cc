...
#include "ns3/nano-sensor-battery.h"
 ...
 NS_LOG_COMPONENT_DEFINE ("NewAdaptiveTDMABattery1");
...
#define MAX_ENERGY 1000 //for nanobattery

int nodedataval[SENSOR_TYPE][DATASIZE]; //array for sensing
...
int datasensetime = 3*slotlen;


uint32_t static numOfDroppedPackets=0;
static int dataindex[NUM_NODE]={};
...
//Scheduling the packets sending from nodes starts here.
	...
	for(int i=starttime; i<finaltime; i=i+slotlen)
	{
		...
		double total_time = (slotlen*NUM_NODE*DATASIZE);
		std::cout<<"\n\nTotal Simulation time: "<<total_time<<" microseconds \n\n";
...
int main (int argc, char *argv[])
{
	...
	Simulator::Schedule(MicroSeconds(endTime-6), & dump_2D_array, nodedataval);
	//Simulator::Schedule(MicroSeconds(endTime-4), & dump_2D_array, Rxnodedataval);
	Simulator::Schedule(MicroSeconds(endTime-2), & count_missed_data, nodedataval, Rxnodedataval, riskc);
	

	Simulator::Stop(MicroSeconds(endTime));
	Simulator::Run();
	Simulator::Destroy();
	
	NS_LOG_UNCOND("\n\nTotal number of Dropped Packets: "<<numOfDroppedPackets);
	NS_LOG_UNCOND("\n\nTotal number of Dropped Risky Data Packets: "<<numOfDroppedRiskData); 
	return 0;
}
...
{	
	...
	std::cout<<"\nTotal number of TXedRisky data: "<<numOfRiskyData<< std::endl;
	...
	std::cout<<"Total number of Received Risky data: "<<numOfReceivedRiskData<<std::endl;
	
	std::cout<<"Total number of slots : "<<total_slot_count<<std::endl;
