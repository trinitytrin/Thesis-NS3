...
/* with Adaptive TDMA (AIMD) protocol with harvesting enabled*/
...
//#include "ns3/nano-sensor-battery.h"
 ...
 NS_LOG_COMPONENT_DEFINE ("NewAdaptiveTDMAHarvesting1");
...
int slotlen=10;
int DataReadinterval = (9)*slotlen;
...
void runAdaptiveTDMA(Ptr<Riskcalculator> riskc)
{
	...
	file_outRX_s << "NewAdaptiveTDMAHarvesting1-Rx_Events";
	...
	file_outTX_s << "NewAdaptiveTDMAHarvesting1-Tx_Events";
...
{	
	...
	std::cout<<"\nFor Gamma: "<<sensor[0]->GetGammaVal() << std::endl;
	std::cout<<"And Gamma_Max: "<<sensor[0]->GetGammaMax() << std::endl;
	...
	resultsfile.open("Results-AdaptiveTDMAHarvesting1.txt", std::ofstream::out | std::ofstream::app);
	resultsfile<<"\n\n~~~~~~~~~ Results for Simulation No: "<<SN<<" ~~~~~~~~~~~~~\n\n";

	resultsfile<<"\nFor Gamma: "<<sensor[0]->GetGammaVal() << std::endl;
	resultsfile<<"And Gamma_Max: "<<sensor[0]->GetGammaMax() << std::endl;
