...
/* Added Tracing here*/
/* With Harvesting component enabled*/
...
/*included interface module functionality - with correct packet broadcasting from interface*/
/* with broadcasted packet recieved and accordingly info changes in the sensor nodes*/
/* with dynamic tdma push-pull scheduling enabled */
/* with new generalized function for sensor nodes for dynamic cycle*/
/* scheduling the sensor nodes*/

/*successfully broadcasting, updating sensor eat rx value,  sensor node schduling is currect with emergency slot also*/
...
//#include "ns3/nano-sensor-battery.h"
 ...
 NS_LOG_COMPONENT_DEFINE ("NewDynamicTDMAHarvesting1");
...
#define MAX_ENERGY (10)*1000 //for harvesting
...
int slotlen=10;
int DataReadinterval = (10)*slotlen;
int datasensetime = 1*slotlen;
int Htime=(10)*slotlen;
...
int finaltime=200035;
int starttime = 25;

int ENDTIME;

Ptr<SensorNodeStatus> sensor[NUM_NODE]; // these are sensors with harvesting enabled
...
void scheduleTxDevices(Ptr<SimpleNanoDevice> devA, Ptr<SimpleNanoDevice> devB, Ptr<Riskcalculator> riskc)
{
	...
	devA->GetMac()->Send(p, devB->GetNode()->GetId());
		...
		if(sensor[n]->ReadyToReadDynamicTDMA(i)==true)
		{
...
void RunDynamicTDMA(Ptr<Riskcalculator> riskc)
{
	NS_LOG_UNCOND("\n"<<Simulator::Now().GetSeconds()<<"\t Inside DynamicTDMA with harvesting function \n ");
	...
	file_outRX_s << "NewDynamicTDMAHarvesting1-Rx_Events";
	...
	file_outTX_s << "NewDynamicTDMAHarvesting1-Tx_Events";
...
//sensor nodes modules creating  
	...
	{
		sensor[i] = Create<SensorNodeStatus>(i, MAX_ENERGY, true, 1, Htime, datasensetime, slotlen); 
...
{	
	...
	for(int i=0; i<NUM_NODE; i++)
	{
		total_recharged = total_recharged + sensor[i]->GetRechargedTimes();;
	}
	avg_recharged = total_recharged/NUM_NODE;
	...
	std::cout<<"\n\nTotal number of recharges : "<<total_recharged<<std::endl;
	std::cout<<"Total avg recharged for each node : "<<avg_recharged<<std::endl;

	std::ofstream resultsfile;
	resultsfile.open("Results-DynamicTDMAHarvesting1.txt", std::ofstream::out | std::ofstream::app);
	...
	resultsfile<<"Maximum Harvested Energy of Each Node = "<<MAX_ENERGY<<std::endl;
	...
	resultsfile<<"Harvesting/Sleep Time Duration = "<<Htime<<std::endl;

	
	resultsfile<<"\nRisky Data Miss Rate = "<<missrate<<"%"<<std::endl;

	resultsfile<<"\nSuccessful TX Simulation ran for: "<<ENDTIME-starttime<< " time units "<<std::endl;
	resultsfile<<"\nTotal number of Recharge Times : "<<total_recharged<<std::endl;
	resultsfile<<"Avg Recharged Times for each node : "<<avg_recharged<<std::endl;
