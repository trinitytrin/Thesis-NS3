...
#include "ns3/nano-sensor-battery.h"
 ...
 NS_LOG_COMPONENT_DEFINE ("NewSimpleTDMABattery1");
...
void scheduleTxDevices(Ptr<SimpleNanoDevice> devA, Ptr<SimpleNanoDevice> devB, Ptr<Riskcalculator> riskc)
{
	...
	if(sensor[n]->ReadyToTxSimpleTDMA(starttime, i, slotlen, NUM_NODE)==true)
	{
		...
		Ptr <Packet> p = sensor[n]->TxDataSimpleTDMA();		
...
void runsimpleTDMA(Ptr<Riskcalculator> riskc)
{
	...
	file_outRX_s << "NewSimpleTDMABattery1-Rx_Events";
	...
	file_outTX_s << "NewSimpleTDMABattery1-Tx_Events";
...
{	
	...
	resultsfile.open("Results-SimpleTDMABattery1.txt", std::ofstream::out | std::ofstream::app);
