...
In this code I tested scheduling by Simple TDMA of two nodes reading data or sensing data and sending data to controller in normal TDMA approach */
...
//#include "ns3/nano-sensor-battery.h"
 ...
 NS_LOG_COMPONENT_DEFINE ("NewSimpleTDMAHarvesting1");
...
void runsimpleTDMA(Ptr<Riskcalculator> riskc);
...
void scheduleTxDevices(Ptr<SimpleNanoDevice> devA, Ptr<SimpleNanoDevice> devB, Ptr<Riskcalculator> riskc)
{
	...
	if(sensor[n]->ReadyToTxSimpleTDMA(starttime, i, slotlen, NUM_NODE)==true)
	{
		std::cout<<"at time "<<i<<": node "<<n<<" is ready to tx\n";
		
		Ptr <Packet> p = sensor[n]->TxData();		
	...
	ENDTIME = i;
		...
		if(sensor[n]->ReadyToReadSimpleTDMA(i)==true)
		{
...
void runsimpleTDMA(Ptr<Riskcalculator> riskc)
{
	NS_LOG_UNCOND("\n"<<Simulator::Now().GetSeconds()<<"\t Inside simpleTDMA function with harvesting \n ");
	...
	file_outRX_s << "NewSimpleTDMAHarvesting1-Rx_Events";
	...
	file_outTX_s << "NewSimpleTDMAHarvesting1-Tx_Events";
...
//sensor nodes modules creating  
	...
	{
		sensor[i] = Create<SensorNodeStatus>(i, MAX_ENERGY, false, 1, Htime, datasensetime, slotlen); 
...
{	
	...
	resultsfile.open("Results-SimpleTDMAHarvesting1.txt", std::ofstream::out | std::ofstream::app);
